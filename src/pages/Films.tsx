...
const Films: FC = () => {
  ...
  const { limit, pageNumber, count } = useSelector((state: RootState) => state.pageView);
  ...
  return (
    <div className="d-flex flex-column justify-content-center align-items-center flex-wrap">
      <div className="d-flex justify-content-center flex-wrap">
        {films.map((film) => (
          <Film key={film.id} id={film.id} description={film.description} img={film.img} title={film.title} />
        ))}
      </div>
      <PaginationList />
    </div>
